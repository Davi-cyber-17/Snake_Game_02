...
/* ---------- Recorde global (key-value.co — sem cadastro) ---------- */
const LEADERBOARD_API = "https://key-value.co/api";
const LEADERBOARD_POLL_MS = 45000; // respeita o limite de 100 leituras/hora

let highScore = 0;
let highScoreHolder = "";
let currentVersion = null; // versão usada para escrita otimista (evita sobrescrever concorrentes)
let leaderboardReady = false;

const isConfigured = leaderboardConfig.token && !leaderboardConfig.token.startsWith("COLE_AQUI");

if (isConfigured) {
    leaderboardReady = true;
    refreshGlobalScore();
    setInterval(refreshGlobalScore, LEADERBOARD_POLL_MS);
} else {
    setSyncStatus("Configure leaderboard-config.js para ativar o recorde global.");
}
...
async function refreshGlobalScore() {
    if (!leaderboardReady) return;

    try {
        const response = await fetch(`${LEADERBOARD_API}/retrieve`, {
            headers: { "X-KV-Token": leaderboardConfig.token }
        });

        if (response.status === 404) {
            // token válido, mas ainda ninguém salvou um recorde
            currentVersion = null;
            setSyncStatus("");
            return;
        }
        ...
        highScore = Number(result.data?.score) || 0;
        highScoreHolder = result.data?.name || "";
...
async function trySaveGlobalScore(finalScore, name) {
    if (!leaderboardReady) return { saved: false, reason: "offline" };

    try {
        // Garante que temos a pontuação/versão mais recente antes de escrever
        await refreshGlobalScore();

        if (finalScore <= highScore) {
            return { saved: false, reason: "beaten" };
        }

        const newData = { score: finalScore, name: name.slice(0, 20) };
        let response;

        if (currentVersion) {
            // Já existe um recorde salvo: usa PATCH com controle de versão.
            // Se alguém salvou entre o refresh e agora, o servidor responde 409.
            response = await fetch(`${LEADERBOARD_API}/store`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    "X-KV-Token": leaderboardConfig.token
                },
                body: JSON.stringify({
            ...
            // Primeiro recorde salvo neste token
            response = await fetch(`${LEADERBOARD_API}/store`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
...
function endGame() {
    ...
    if (leaderboardReady && score > 0 && score > highScore) {
